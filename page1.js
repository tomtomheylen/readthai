function togglePopup(element) {
  ...
  function createMarkElements(){//for marks and tones
    const container = document.getElementById('markContainer');
    container.innerHTML = '';
  
  
  
    toneMarks.forEach(toneMark => {
      const div = document.createElement('div');
      div.className = 'consonant';
  
      const innerBox1 = document.createElement('div');
      innerBox1.className = 'M-inner-box1';
      innerBox1.textContent = toneMark.symbol;
      innerBox1.addEventListener('click', function() {
        popupMarks(toneMark.symbol, toneMark.description, toneMark.words);
      });  

      div.appendChild(innerBox1);
  
      const innerBox2 = document.createElement('div');
      innerBox2.className = 'M-inner-box2';
      innerBox2.id = toneMark.symbol;
      //select the first 2 words from descripton and display in innerbox2
      innerBox2.innerHTML = '<b>'+toneMark.description.split(' ').slice(0, 2).join('<br>') + '</b>';
 ...
 function closepopupMarks(){
    document.getElementById('popupMarks').style.display = 'none';
 };
  
 function popupMarks(symbol, description, words){
  var container = document.getElementById('popupMarks');
  container.style.display = 'block';
  container.innerHTML = '';
  container.innerHTML = '<span class="pop-close-btn" onclick="closepopupMarks()">X</span>';

  var html = '<div class="M-symbol"><center>'+symbol+'</center></div>';
  html += '<div class="pop-pronounce"><b>'+ description + '</b></div>';
  html += '<b>Common words</b><br>'+ words.join('<br>');

  //symbol+'<br>'+description+'<br>'+words
  ...
  function playSound(audioUrl) {
    ...
    getAssetURL('/'+audioUrl, function(url) {
      new Audio(url).play();
