...
function initTabs() {
    ...
    addTabButton.innerHTML = '<div style="font-size: 35px; font-weight: bold;">+</div>';
...
function editTabTitle(event) {
    ...
    tabButton.addEventListener('keydown', function(e) {
        ...
        if (e.key === 'Backspace') {
            ...
            // If the cursor is right before the delete button
            if (selection.anchorNode && selection.anchorNode.nextSibling && selection.anchorNode.nextSibling.className === 'delete-tab') {
                e.preventDefault(); // Prevent the default action of backspace
                
                // Add an empty space before the delete button
                var spaceNode = document.createTextNode(" ");
                tabButton.insertBefore(spaceNode, selection.anchorNode.nextSibling);
                
                // Keep the cursor in place
                var range = document.createRange();
                range.setStartBefore(selection.anchorNode.nextSibling);
                range.collapse(true);
                selection.removeAllRanges();
                selection.addRange(range);
            }
        ...
        if (e.key === 'Delete') {
            ...
            // If the cursor is right after the content and right before the delete button
            if (selection.anchorNode && selection.anchorNode.nextSibling && selection.anchorNode.nextSibling.className === 'delete-tab') {
                e.preventDefault(); // Prevent the default action of delete
...
function saveTabs() {
    ...
    tabButtons.forEach((button, index) => {
        ...
        // Fetch the title and check if it's empty or just a space
        var title = button.childNodes[0].nodeValue.trim();
        if (!title || title === ' ') {
            title = "Tab " + (index + 1);
        }
