function changeFont(font) {
      var elements = document.getElementsByClassName('inner-box1');
      ...
      var elements = document.getElementsByClassName('vowel-inner-box1');
      for (var i = 0; i < elements.length; i++) {
        if(font == 'Sarabun'){
          elements[i].style.fontFamily = '';
        }else{
...
const toggleDarkMode = () => {
  ...
  if (root.classList.contains('dark-mode')) {
    ...
    darkModeText.textContent = 'White';
  }
};
...
//   document.getElementById('pop-write').textContent = img ? 'Write' : '';
  ...
  document.getElementById('popup').appendChild(closeBtn);
...
function adjustConsonantContainerPadding() {
  ...
  var consonantContainer = document.getElementById('consonantContainer');
...
function togglePopupVowel(element){
  //alert(element.id);
}
...
// Call createElements() initially to create elements based on the default counter value
createElements();
makeFirstLetterBold();
createVowelElements();
